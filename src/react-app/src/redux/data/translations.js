const translations = {
  vi: {
    home: {
      welcome: "Chào mừng",
      example: "Đây là một ví dụ",
      description:
        "Chúng tôi đang sử dụng Redux để quản lý trạng thái ngôn ngữ.",
    },
    profile: {
      nameUser: "Phuc Nè",
      age: "Tuổi",
      email: "Email",
    },
    about: {
      description: "Chào mừng bạn đến với trang giới thiệu.",
      contact: "Liên hệ chúng tôi",
    },
    header: {
      store: "Kho hàng",
      support: "Hỗ trợ",
      language: "Ngôn ngữ",
      download: "Tải xuống",
      searchStore: "Tìm kiếm sản phẩm",
    },
    navbar: {
      SearchStore: "Tìm kiếm sản phẩm",
      Discover: "Khám phá",
      Products: "Sản phẩm",
      News: "Tin tức",
      Wishlist: "Yêu thích",
      Cart: "Giỏ hàng",
    },
    homeCarouselHead: {
      BuyNow: "Mua ngay",
      AddToCart: "Thêm vào giỏ hàng",
    },
    home: {
      Adventure: "Cuộc phiêu lưu",
      Simulation: "Mô phỏng",
      RPG: "RPG",
      BuyNow: "Mua Ngay",
      BestSellers: "Bán chạy nhất",
      HighestValue: "Có giá trị cao nhất",
      MostPopularProducts: "Sản phẩm được yêu thích nhất",
    },
    homeProductCarousel: {
      AddToCart: "Thêm vào giỏ hàng",
      AddToWish: "Thêm vào yêu thích",
    },
    footer: {
      support: "Hỗ trợ khách hàng",
      distribute: "Phân phối tại Epic Game",
      careers: "Tuyển dụng",
      company: "Giới thiệu về công ty",
      fanArtPolicy: "Chính sách Fan Art",
      uxResearch: "Nghiên cứu UX",
      storeEula: "Điều khoản sử dụng cửa hàng",
      onlineServices: "Dịch vụ trực tuyến",
      communityRules: "Quy định cộng đồng",
      newsroom: "Tin tức Epic Game",
      madeBy: "Được phát triển bởi Epic Game",
      termsOfService: "Điều khoản dịch vụ",
      privacyPolicy: "Chính sách quyền riêng tư",
      storeRefundPolicy: "Chính sách hoàn tiền cửa hàng",
      copyright:
        "© 2024, Epic Game. Tất cả quyền được bảo lưu. Epic Game và logo Epic Game là các thương hiệu hoặc nhãn hiệu đã đăng ký của Epic Game tại Việt Nam và các quốc gia khác.",
      resources: "Nguồn lực",
    },
    products: {
      brands: "Thương hiệu",
      material: "Chất liệu",
      color: "Màu sắc",
      Size: "Kích cỡ",
      Style: "Phong cách",
      SearchProduct: "Tìm kiếm sản phẩm",
      SeeAll: "Xem tất cả",
      purpose: "Mục đích",
    },
    profile: {
      UserInfo: "Thông tin người dùng",
      Oder: "Đơn hàng",
      PasswordAndSetting: "Mật khẩu & Cài đặt",
      NeedHelp: "Bạn cần giúp đỡ?",
      HistoryBuy: "Lịch sử mua hàng",
      LibraryGame: "Thư viện game",
    },
    register: {
      createAccount: "Tạo Tài Khoản",
      dateOfBirthPrompt:
        "Vui lòng nhập ngày sinh của bạn. Điều này giúp bạn có trải nghiệm an toàn và thú vị ở bất kỳ độ tuổi nào.",
      dateAndTimeOfBirth: "Ngày và Giờ Sinh",

      alreadyHaveAccount: "Đã có tài khoản?",
      signIn: "Đăng nhập",
      privacyPolicy: "Chính Sách Quyền Riêng Tư",
      continue: "Tiếp tục",
    },
  },

  // --------------------ENGLISH--------------------
  en: {
    home: {
      welcome: "Welcome",
      example: "This is an example",
      description: "We are using Redux to manage language state.",
    },
    profile: {
      nameUser: "Phuc Nè",
      age: "Age",
      email: "Email",
    },
    about: {
      description: "Welcome to the about page.",
      contact: "Contact us",
    },
    header: {
      store: "Store",
      support: "Support",
      language: "Language",
      download: "Download",
      searchStore: "Search products",
    },
    navbar: {
      SearchStore: "Search products",
      Discover: "Discover",
      Products: "Products",
      News: "News",
      Wishlist: "Wishlist",
      Cart: "Cart",
    },
    homeCarouselHead: {
      BuyNow: "Buy Now",
      AddToCart: "Add to Cart",
    },
    home: {
      Adventure: "Adventure",
      Simulation: "Simulation",
      RPG: "RPG",
      BuyNow: "Buy Now",
      BestSellers: "Best Sellers",
      HighestValue: "Highest Value",
      MostPopularProducts: "Most Popular Products",
    },
    homeProductCarousel: {
      AddToCart: "Add to Cart",
      AddToWish: "Add to Wishlist",
    },
    footer: {
      support: "Customer Support",
      distribute: "Distributed at Epic Game",
      careers: "Careers",
      company: "About the Company",
      fanArtPolicy: "Fan Art Policy",
      uxResearch: "UX Research",
      storeEula: "Store EULA",
      onlineServices: "Online Services",
      communityRules: "Community Rules",
      newsroom: "Epic Game News",
      madeBy: "Made by Epic Game",
      termsOfService: "Terms of Service",
      privacyPolicy: "Privacy Policy",
      storeRefundPolicy: "Store Refund Policy",
      copyright:
        "© 2024, Epic Game. All rights reserved. Epic Game and the Epic Game logo are trademarks or registered trademarks of Epic Game in Vietnam and other countries.",
      resources: "Resources",
    },
    products: {
      brands: "Brands",
      material: "Material",
      color: "Color",
      Size: "Size",
      Style: "Style",
      SearchProduct: "Search Products",
      SeeAll: "See All",
      purpose: "Purpose",
    },
    profile: {
      UserInfo: "User Information",
      Oder: "Order",
      PasswordAndSetting: "Password & Settings",
      NeedHelp: "Need Help?",
    },
    register: {
      createAccount: "Create Account",
      dateOfBirthPrompt:
        "Please enter your date of birth. This is to help you have a safe and fun experience whatever your age.",
      dateAndTimeOfBirth: "Date and Time of Birth",
      continue: "Continue",
      alreadyHaveAccount: "Already have an account?",
      signIn: "Sign in",
      privacyPolicy: "Privacy Policy",
    },
  },
  // --------------------TÂY BÀN NHA--------------------
  es: {
    home: {
      welcome: "Bienvenido",
      example: "Este es un ejemplo",
      description:
        "Estamos utilizando Redux para gestionar el estado del idioma.",
    },
    profile: {
      nameUser: "Phuc Nè",
      age: "Edad",
      email: "Correo electrónico",
    },
    about: {
      description: "Bienvenido a la página de información.",
      contact: "Contáctenos",
    },
    header: {
      store: "Tienda",
      support: "Soporte",
      language: "Idioma",
      download: "Descargar",
      searchStore: "Buscar productos",
    },
    navbar: {
      SearchStore: "Buscar productos",
      Discover: "Descubrir",
      Products: "Productos",
      News: "Noticias",
      Wishlist: "Lista de deseos",
      Cart: "Carrito",
    },
    homeCarouselHead: {
      BuyNow: "Comprar ahora",
      AddToCart: "Añadir al carrito",
    },
    home: {
      Adventure: "Adventure",
      ProductsGirl: "Productos para niñas",
      Simulation: "Simulation",
      RPG: "RPG",
      BuyNow: "Comprar ahora",
      BestSellers: "Más vendidos",
      HighestValue: "Mayor valor",
      MostPopularProducts: "Productos más populares",
    },
    homeProductCarousel: {
      AddToCart: "Añadir al carrito",
      AddToWish: "Añadir a la lista de deseos",
    },
    footer: {
      support: "Soporte al cliente",
      distribute: "Distribuido en Epic Game",
      careers: "Carreras",
      company: "Acerca de la empresa",
      fanArtPolicy: "Política de Fan Art",
      uxResearch: "Investigación de UX",
      storeEula: "EULA de la tienda",
      onlineServices: "Servicios en línea",
      communityRules: "Reglas de la comunidad",
      newsroom: "Noticias de Epic Game",
      madeBy: "Hecho por Epic Game",
      termsOfService: "Términos del servicio",
      privacyPolicy: "Política de privacidad",
      storeRefundPolicy: "Política de reembolsos de la tienda",
      copyright:
        "© 2024, Epic Game. Todos los derechos reservados. Epic Game y el logo de Epic Game son marcas comerciales o marcas registradas de Epic Game en Vietnam y otros países.",
      resources: "Recursos",
    },
    products: {
      brands: "Marcas",
      material: "Material",
      Color: "Color",
      Size: "Tamaño",
      Style: "Estilo",
      SearchProduct: "Buscar productos",
      SeeAll: "Ver todo",
      purpose: "Propósito",
    },
    profile: {
      UserInfo: "Información del usuario",
      Oder: "Pedido",
      PasswordAndSetting: "Contraseña y configuración",
      NeedHelp: "¿Necesitas ayuda?",
    },
    register: {
      createAccount: "Crear Cuenta",
      dateOfBirthPrompt:
        "Por favor, ingresa tu fecha de nacimiento. Esto es para ayudarte a tener una experiencia segura y divertida a cualquier edad.",
      dateAndTimeOfBirth: "Fecha y Hora de Nacimiento",
      continue: "Continuar",
      alreadyHaveAccount: "¿Ya tienes una cuenta?",
      signIn: "Iniciar sesión",
      privacyPolicy: "Política de Privacidad",
    },
  },
  // Thêm các ngôn ngữ và trang khác tại đây
};

export default translations;
